...
- If another check's fix removes one of your violations before your own `fix()` ever runs, `CheckOrchestrator` reports it as `[RESOLVED INDIRECTLY]` on your behalf — your check needs no awareness of the other check, and must not try to detect this itself. See `docs/adr/0053-indirect-resolution-outcome.md`.
