...
Both rules are evaluated in `ruff`'s own glob dialect, which is a third thing this decision has to get right and which `Path.full_match` does not implement: `*` and `?` span path separators, `**` standing alone as a path component also matches zero components, and braces alternate. A pattern is resolved against its anchor before the anchored rule is applied, and left exactly as written for the unanchored one — `ruff` resolves a pattern against its anchor but matches a bare name literally, so `./tests/**` covers the whole `tests/` subtree while `./mod.py` covers the root's own `mod.py` and no other. Resolution covers `.`, `..`, repeated separators, and an already-absolute pattern; one that resolves outside its anchor matches nothing, which is the same rule stated below for files. It is textual, as `ruff`'s is: the resolved pattern stays a single glob, matched against a file's own absolute path, rather than being expanded into one pattern per alternation branch — which would be exponential in the number of branches. The anchor is escaped on the way in, since it is a directory path rather than a pattern. Alternation carries a ceiling: a pattern spelling out more than 1024 ways to match is refused as invalid configuration. Python's regex engine explores alternation by backtracking, so overlapping branches repeated enough times take seconds per file where `ruff`'s automaton takes microseconds, and a matcher of our own written to be linear would cost every ordinary pattern far more than the ceiling costs a pathological one. `ruff` is the reference for the pattern language as much as for the two rules above — a pattern copied between the two tools has to cover the same files, and `src/*.py` covering only the direct children of `src/` in one tool and its whole subtree in the other is exactly the trap this decision exists to close. The dialect is shared with `per-file-ignores` (ADR-0049), which applies its own matching rules on top of it.
...
- A pattern with more than 1024 alternative spellings is refused, which is the one place a pattern `ruff` accepts is rejected here. Nothing written to name real files comes near it, and the alternative is a run that stalls on every file the pattern is tried against.
