...
- A nested `def`'s decorators and parameter defaults still count as the enclosing function's own evidence, since they evaluate when the `def` statement runs — same as a decorator's own call arguments or a class's base/keyword arguments. A nested `def`'s parameter and return annotations do not: PEP 649, default behavior on this project's supported Python version, defers annotation evaluation until something reads `__annotations__`, so an annotation expression on a nested function never runs as part of the enclosing function's own execution.
