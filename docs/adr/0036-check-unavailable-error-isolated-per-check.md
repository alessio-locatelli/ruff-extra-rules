...
- **Make TRI006 opt-in only, excluded from the default "run everything" set**: would need a new "default-enabled" concept on `ASTCheck`/the CLI that doesn't exist today, and would apply this fix only to TRI006 rather than to any future check that can raise `CheckUnavailableError`.
