...
- `README.md`'s TRI005 section gains an explicit line documenting the test-directory relaxation — its absence was flagged during this review as easy to forget, since nothing in the user-facing docs currently mentions it.
- A related, separate idea surfaced during this same discussion and is _not_ part of this decision: this repo's own self-dogfooding config (`.pre-commit-config.yaml`) currently excludes `tests/` entirely (`files: ^src/`) from every check, not just TRI005. Enabling `ast-checks` against `tests/` in this repo's own pre-commit config is a distinct future feature — tracked separately in `docs/work-items/10-lint-tests-directory.md` — that would need to reconcile with `tests/fixtures/` containing deliberately "bad" example code and the test suite's own idiomatic use of forbidden names.
- Tracked as `docs/work-items/08-document-is-test-file-heuristic.md`.
