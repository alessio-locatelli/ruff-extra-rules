...
- The classification is by count, not by identity, so it is accurate about _how many_ of a check's violations were resolved by another fix and only best-effort about _which_. When a check's messages are identical or drift, the named violation can be the wrong one of that check's own set.
- One case the count cannot see: a fix that removes one of another check's violations _and_ introduces a new one for that same check leaves the count unchanged, so the removed violation gets no outcome and the new one is reported on its own. Deciding purely by message instead, so that a leftover replacement never absorbs a removed violation, was measured against the shipped checks and is worse: `meaningless-vars` renaming a variable rewrites the text of `redundant-assignment`'s message for the very same violation, which then reports that violation twice — once truthfully `[FIXED]`, once falsely `[RESOLVED INDIRECTLY]`. A false claim is worse than a missing line, so the count wins where the two disagree. Closing this properly needs per-violation identity, rejected above.
