...
Also run it from inside the project being checked. `ty` resolves its own workspace root from the current working directory, so invoking this check's CLI directly (rather than through prek/pre-commit, which always runs a hook from the repo it's checking) against a file outside the current directory points `ty` at the wrong project — it won't see that project's own dependencies or configuration, which can change its diagnostics. If a file falls entirely outside the resolved project root, this check can't get any diagnostics for it at all; rather than treat that silence as "nothing to report," it skips the file and logs a warning explaining why. The same applies if `ty`'s own project configuration excludes the file from its checked scope (for example, a project that keeps `tests/` outside `ty`'s enforcement surface) — this check verifies `ty` is actually checking a file before trusting its silence, and skips the file with a warning otherwise. That verification probes two unrelated diagnostic rules (an invalid assignment and an invalid argument type); if a project's `ty` config disables both of those specific rules, the probe can't tell that apart from the file being out of scope, and this check skips the file with the same warning even though `ty` is actually checking it. This is a known, accepted limitation — disabling both rules at once is not a configuration we've seen in practice.
...
`ty`'s diagnostics can't see a difference between two types that produce different `repr()`/`str()` output once interpolated — a `frozenset` and a `tuple` are equally valid to embed in an f-string as far as type-checking is concerned, even though their literal text differs (`frozenset({'a'})` vs `('a',)`). When a conversion's result — directly, or via the variable it's assigned to (including a chain of plain reassignments, an attribute or subscript target's own base name, or a value nested inside an assigned list/dict/tuple literal) — is later interpolated into an f-string, passed to `str.format()`, or supplied to `%`-formatting (recognized when the format string is a literal, an f-string, or a name bound to one), and the wrapped value isn't already exactly the constructor's type, this check leaves it alone rather than risk changing what actually gets embedded in the string. A format string built by concatenating or otherwise composing pieces at runtime (`fmt = "%s" + suffix`) isn't traced back to a literal, so a conversion feeding that composed string's formatting is outside this guard's reach — as with any static analysis, prefer a `conservative`-level report, or review an `aggressive`-only one, when the surrounding code is this dynamic.

At `aggressive` level, a `list`/`dict`/`set`/`bytearray` conversion assigned to a name (directly, through a chain of plain reassignments, through any depth of attribute/subscript target, or through a `for`/comprehension target — including one that destructures a same-shaped tuple/list literal element) that's later mutated in place — a subscript assignment or deletion, an augmented assignment, or a call to a mutating method such as `.append()`, `.update()`, or `.add()` — is never reported: removing the conversion there would turn a distinct copy into a shared, mutable reference to the original value, changing what the mutation actually affects. A `for`/comprehension target over a non-literal iterable, or a `with` target (none of these four constructors are context managers), isn't traced this way and stays subject to `aggressive` mode's usual aliasing/mutation caveat.

At `aggressive` level, `set(...)`/`frozenset(...)` is never reported as redundant when the wrapped value's own type permits duplicates (a `list`, `tuple`, or any other type without set semantics) — converting a duplicate-permitting value is deduplication, a real behavior change `ty`'s type-only view can't see, not redundant wrapping. It's still reported when the wrapped value already has set semantics: an exact `set`/`frozenset` match, or a `dict`'s keys (`set(a_dict.keys())`), since a dict can't have stored duplicate keys in the first place.

At `aggressive` level, a conversion that isn't already an exact type match is never reported when it's a direct positional or keyword argument of a call to a logging method (`.debug()`, `.info()`, `.warning()`, `.warn()`, `.error()`, `.exception()`, `.critical()`, `.fatal()`, on any receiver): a structured logger commonly renders its raw argument differently from the converted form (e.g. `logger.warning("msg", path=str(a_path))` loses the distinct `PosixPath(...)` rendering a logging backend would otherwise show for `a_path` itself). A value that only reaches the logging call indirectly — through an intermediate variable, a wrapper function, or a container literal built inline — is outside this guard's reach and stays subject to `aggressive` mode's usual weakly-typed-sink caveat.
