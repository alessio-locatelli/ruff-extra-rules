...
At `aggressive` level, `set(...)`/`frozenset(...)` is never reported as redundant when the wrapped value's own type permits duplicates (a `list`, `tuple`, or any other type without set semantics) unless the wrapped value is already exactly a `set`/`frozenset`-family type — converting a duplicate-permitting value is deduplication, a real behavior change `ty`'s type-only view can't see, not redundant wrapping.
