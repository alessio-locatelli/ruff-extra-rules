...
At `aggressive` level, a `list`/`dict`/`set`/`bytearray` conversion assigned to a name (directly, through a chain of plain reassignments, or through an attribute/subscript target's own base name) that's later mutated in place — a subscript assignment or deletion, an augmented assignment, or a call to a mutating method such as `.append()`, `.update()`, or `.add()` — is never reported: removing the conversion there would turn a distinct copy into a shared, mutable reference to the original value, changing what the mutation actually affects. A copy stored two levels deep (an attribute holding a container that's later mutated by subscript, for example) or captured by a `for`/`with`/comprehension target isn't traced this way and stays subject to `aggressive` mode's usual aliasing/mutation caveat.
